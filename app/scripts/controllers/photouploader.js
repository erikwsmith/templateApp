...
angular.module('mts.fieldbook')
  .controller('PhotoUploadController', ['$scope','$rootScope', '$q', 'photoService', 'Ref', 'Storage','$firebaseObject', '$routeParams', 'Tract','Weld', 'Lightbox', 'IMAGETYPES', 'WORKSPACE', 'WeldImages',
    function ($scope,$rootScope,$q,photoService, Ref, Storage, $firebaseObject, $routeParams, Tract, Weld, Lightbox, IMAGETYPES, WORKSPACE, WeldImages) {

    new Tract($routeParams.tractid).$bindTo($scope, 'tract');

    $scope.local = {};
    var weldImagesReference = Ref.child('images').child($routeParams.tractid);
      var docStorageReference = Storage.child('images').child($routeParams.tractid);
    $scope.local.weldImages = new WeldImages(weldImagesReference);


    if ($routeParams.tractid !== photoService.getParentID() && photoService.getParentID() !== '') {
      photoService.clearAllFiles();
    }

    var fileType = (typeof($routeParams.fileType) === 'undefined') ? 'images' : $routeParams.fileType;

    photoService.setPath($routeParams.tractid, fileType);
