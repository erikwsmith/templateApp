...
.controller('EnvironmentalPermitsCtrl', ['$firebaseArray','$firebaseObject','$firebaseAuth','$scope', '$location','$timeout', 'Ref', 'LOCATIONS', 'user', 'Auth', 'ROW','WORKSPACE',
  ...
  var permitsRefFiltered =  Ref.child('environmentalpermits').orderByChild('inactive').startAt(null).endAt(false);
  var permitsRef =  Ref.child('environmentalpermits');
...
$scope.orderBy = function(fld, e) {
    ...
    $scope.addNewPermit = function(){
      var p = Ref.child('environmentalpermits').push({
        group: 'environmental_permits'
      });
      $location.path('/environmental_permits/'+p.key);
    };
  ...
  .factory('EnvironmentalPermit', ['$firebaseObject', 'Ref', 'ROW',
    function($firebaseObject, Ref, ROW) {
      return function(environmentalpermitid) {
        ...
        var p = environmentalpermitid;
        if (environmentalpermitid === "new") {
          p = Ref.child('environmentalpermits').push().key;

        }
        var permitRef = Ref.child('environmentalpermits').child(p);
  ...
  //   }
  // ])
  .controller('EnvironmentalPermitCtrl', ['$scope', '$rootScope', 'Ref', 'Storage', '$q', '$firebaseObject', '$firebaseArray', '$timeout',
      '$routeParams', 'EnvironmentalPermit', '$window', '$location', 'user', 'LOCATIONS', 'openPost', 'ROW', 'WeldImages',
    function ($scope, $rootScope, Ref, Storage, $q, $firebaseObject, $firebaseArray, $timeout,
      ...
      var inactiveRef = Ref.child('environmentalpermits').child($routeParams.environmentalpermitid);
      var inactiveStatus = inactiveRef.child('inactive');
      $scope.inactiveStatus = $firebaseObject(inactiveStatus);


      Ref.child('environmentalpermits').on('value',function(snapshot) {
        var obj = $firebaseArray(Ref.child('environmentalpermits'));
            obj.$loaded()
              .catch(alert)
              .then(function() {
      ...
      Ref.child('agents').on('value',function(snapshot) {
        ...
        var obj = $firebaseArray(Ref.child('agents').orderByChild('department').equalTo('ENVIRONMENTAL'));
