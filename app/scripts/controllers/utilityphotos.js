...
angular.module('mts.fieldbook')
  .controller('UtilityPhotosCtrl', ['$scope','$rootScope', '$q', 'photoService', 'Ref', 'Storage','$firebaseObject', '$routeParams', 'UtilityPermit','Weld', 'Lightbox', 'IMAGETYPES', 'WORKSPACE', 'WeldImages',
    function ($scope,$rootScope,$q,photoService, Ref, Storage, $firebaseObject, $routeParams, UtilityPermit, Weld, Lightbox, IMAGETYPES, WORKSPACE, WeldImages) {

    new UtilityPermit($routeParams.utilitypermitid).$bindTo($scope, 'permit');

    $scope.local = {};
    var weldImagesReference = Ref.child('images').child($routeParams.utilitypermitid);
      var docStorageReference = Storage.child('images').child($routeParams.utilitypermitid);
    $scope.local.weldImages = new WeldImages(weldImagesReference);


    if ($routeParams.utilitypermitid !== photoService.getParentID() && photoService.getParentID() !== '') {
      photoService.clearAllFiles();
    }

    var fileType = (typeof($routeParams.fileType) === 'undefined') ? 'images' : $routeParams.fileType;

    photoService.setPath($routeParams.utilitypermitid, fileType);
