...
angular.module('mts.fieldbook')
  .controller('TractDocsController', ['$scope','$rootScope', '$q', 'fileService', 'Ref', 'Storage','$firebaseObject', '$routeParams', 'Tract','Weld', 'Lightbox', 'IMAGETYPES', 'WORKSPACE', 'WeldImages',
    function ($scope,$rootScope,$q,fileService, Ref, Storage, $firebaseObject, $routeParams, Tract, Weld, Lightbox, IMAGETYPES, WORKSPACE, WeldImages) {

    new Tract($routeParams.tractid).$bindTo($scope, 'tract');

    $scope.local = {};
    var weldImagesReference = Ref.child('documents').child($routeParams.tractid);
      var docStorageReference = Storage.child('documents').child($routeParams.tractid);
    $scope.local.weldImages = new WeldImages(weldImagesReference);


    if ($routeParams.tractid !== fileService.getParentID() && fileService.getParentID() !== '') {
      fileService.clearAllFiles();
    }

    var fileType = (typeof($routeParams.fileType) === 'undefined') ? 'documents' : $routeParams.fileType;

    fileService.setPath($routeParams.tractid, fileType);
