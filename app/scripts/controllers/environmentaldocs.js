...
angular.module('mts.fieldbook')
  .controller('EnvironmentalDocsCtrl', ['$scope','$rootScope', '$q', 'fileService', 'Ref', 'Storage','$firebaseObject', '$routeParams', 'EnvironmentalPermit','Weld', 'Lightbox', 'IMAGETYPES', 'WORKSPACE', 'WeldImages',
    function ($scope,$rootScope,$q,fileService, Ref, Storage, $firebaseObject, $routeParams, EnvironmentalPermit, Weld, Lightbox, IMAGETYPES, WORKSPACE, WeldImages) {

    new EnvironmentalPermit ($routeParams.environmentalpermitid).$bindTo($scope, 'permit');

    $scope.local = {};
    var weldImagesReference = Ref.child('documents').child($routeParams.environmentalpermitid);
      var docStorageReference = Storage.child('documents').child($routeParams.environmentalpermitid);
    $scope.local.weldImages = new WeldImages(weldImagesReference);


    if ($routeParams.environmentalpermitid !== fileService.getParentID() && fileService.getParentID() !== '') {
      fileService.clearAllFiles();
    }

    var fileType = (typeof($routeParams.fileType) === 'undefined') ? 'documents' : $routeParams.fileType;

    fileService.setPath($routeParams.environmentalpermitid, fileType);
